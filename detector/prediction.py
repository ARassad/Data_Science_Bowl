    ...
    for n, id_ in tqdm(enumerate(ids), total=len(ids)):
        try:
            ...
            for i in range(len(images)):
                ...
                square_for_detector = np.array([square_for_detector[i] for i in range(len(square_for_detector))
                                                  if is_nucl[i][0] > TRESHHOLD_FIRST_DETECTOR])
                bounds = [bounds[i] for i in range(len(bounds)) if is_nucl[i][0] > TRESHHOLD_FIRST_DETECTOR]

                # Говорит, что в квадрате только одна клетка
                if len(square_for_detector) > 0:
                    is_one_nucl = detector_second.predict(square_for_detector)

                    bounds = [bounds[i] for i in range(len(bounds)) if is_one_nucl[i][0] > TRESHHOLD_SECOND_DETECTOR]
                    for box in bounds:
                        bbs.append((box[0] - SIZE_IMAGE[0], box[1] - SIZE_IMAGE[1], box[0], box[1]))
